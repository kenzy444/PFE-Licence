...
$_SESSION['MAT']="";
$tsql="select * from [usthb90000L].[dbo].[CLASSEMENTL] where mat='$mat' and ocode='$ocode' and ANET=$annee order by Rang";
                $getresults2 = $conn->prepare($tsql);
            $getresults2->execute();
            $results = $getresults2->fetchAll(PDO::FETCH_BOTH);
$tsql="select count(*) as c from [usthb90000L].[dbo].[CLASSEMENTL] where ocode='$ocode' and ANET=$annee";
            $getresults2 = $conn->prepare($tsql);
        $getresults2->execute();
...
<head>
  <title>USTHB - Generation classement licence - Etudiant</title>
...
<body>
 ...
 <?php 
 if(sizeof($results)==0){
    $tsql="delete from [usthb90000L].[dbo].[CLASSEMENTL] where ocode='$ocode' and ANET=$annee";
    $getresults2 = $conn->prepare($tsql);
    $getresults2->execute();
    ?> 
